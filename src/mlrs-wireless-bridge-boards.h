...
//-- ESP32-C3 Super Mini (nologo)
#elif defined MODULE_ESP32C3_SUPER_MINI
    ...
    #ifdef UNIT_GROUND
        // Ground Unit: Use USB-Serial for MAVLink data to PC
        #undef USE_SERIAL1_DBG
        // No SERIAL_RXD/TXD defined = uses default Serial (USB)
    #else
        // Air Unit: Use Serial1 (GPIO20/21) for MAVLink data to FC
        #define USE_SERIAL1_DBG
        #define SERIAL_RXD 20 // = RX
        #define SERIAL_TXD 21 // = TX
    #endif

    #undef LED_IO
    #define LED_IO  8  // Built-in LED on GPIO8
    #define USE_LED

    #undef GPIO0_IO
    #define GPIO0_IO  9  // Boot button on GPIO9
    // WIRELESS_PROTOCOL is set via build_flags in platformio.ini
    // Air Unit: 1 (UDP AP), Ground Unit: 2 (UDPSTA)


//-- ESP32 DevKit (Ground Unit with classic Bluetooth SPP)
#elif defined MODULE_ESP32_DEVKIT
    ...
    #undef USE_SERIAL1_DBG
    #undef USE_SERIAL2_DBG
    // ESP-NOW + Bluetooth mode: no serial needed for MAVLink, uses Bluetooth

    #undef LED_IO
    #define LED_IO  2  // Built-in LED on GPIO2
    #define USE_LED

    #undef GPIO0_IO
    // GPIO0 is boot button, don't use for AT mode to keep things simple
